...
wxWindow *ViewSectionHeaders::doCreateWindow(wxWindow *parent, wxWindowID id)
{
	...
	if(GetFile()->GetHeader()->e_shnum > 0) {
		...
		for(int i=1; i<GetFile()->GetHeader()->e_shnum - 1;i++) {
			...
			mTable->SetCell(rowStart + 2, 1, FlagManager::GetDescription(FlagManager::SetSectionType, header->sh_type), 
				Location::BuildFlagLocation(FlagManager::SetSectionType, header->sh_type));
